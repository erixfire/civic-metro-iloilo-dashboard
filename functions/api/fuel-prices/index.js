 ...
 *
 * Moved from functions/api/fuel-prices.js to functions/api/fuel-prices/index.js
 * to allow the /sync sub-route to coexist.
 */
...
export async function onRequest(ctx) {
  ...
  if (method === 'GET') {
    ...
    return json({
      ...
      philippines: { gasoline: latest.ph_gasoline_avg, diesel: latest.ph_diesel_avg },
    })
  }
  ...
  if (method === 'POST') {
    ...
    const {
      ...
      ph_gasoline_avg, ph_diesel_avg, logged_by, source,
    } = body
    if (!as_of) return json({ error: 'as_of date is required' }, 400)
    await env.DB.prepare(
      `INSERT INTO fuel_prices (
         as_of, iloilo_gasoline_avg, iloilo_gasoline_min, iloilo_gasoline_max,
         iloilo_diesel_avg, iloilo_diesel_min, iloilo_diesel_max,
         iloilo_kerosene_avg, iloilo_kerosene_min, iloilo_kerosene_max,
      ...
      ph_gasoline_avg     ?? null, ph_diesel_avg       ?? null,
      source ?? 'LPCC · Manual', logged_by ?? null,
    ).run()
