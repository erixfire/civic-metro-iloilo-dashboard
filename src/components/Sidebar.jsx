...
export default function Sidebar() {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-full z-40 bg-white dark:bg-zinc-900 border-r border-black/10 dark:border-white/10 shadow-lg transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-56' : 'w-0 overflow-hidden'}`}>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto scrollbar-thin">
        {NAV_ITEMS.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg mb-1 text-sm font-medium transition-colors text-left ${activeSection === item.id ? 'bg-brand-600 text-white' : 'text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
          >
