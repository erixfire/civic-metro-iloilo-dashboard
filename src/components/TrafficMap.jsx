 ...
 * Mobile-optimised: map heights adapt to screen size, tile load is lazy.
 * Keyboard accessible tab switcher with proper ARIA roles.
 */
...
function makeIncidentIcon(type, severity) {
  ...
  return L.divIcon({
    html: `<div style="background:${bg};border:2px solid rgba(0,0,0,.25);border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;" role="img" aria-label="${type} incident">${emoji}</div>`,
...
// Detect mobile to reduce tile quality for slow connections
  ...
  return (
    <div className="relative">
      <iframe
        ...
        allowFullScreen
        loading="lazy"
        title="Waze Live Traffic map for Iloilo City"
      />
...
function CityOpsTab({ incidents }) {
  ...
  // Use CartoDB Positron on mobile (lighter tiles, fewer labels = faster on 4G)
  ...
  return (
    <div role="region" aria-label="City operations map showing traffic and incidents">
      <MapContainer
...
const TABS = [
  { id: 'waze',    label: '🚦 Waze Live', sub: 'Trapiko · Live' },
  { id: 'cityops', label: '🗺️ City Ops',   sub: 'Insidente · Ulan' },
]
  ...
  return (
    <div className="rounded-xl border border-black/10 dark:border-white/10 overflow-hidden shadow-sm bg-white dark:bg-zinc-900">
      ...
      <div
        id="tabpanel-waze"
        role="tabpanel"
        aria-labelledby="tab-waze"
        hidden={activeTab !== 'waze'}
      >
        <WazeTab />
      </div>
      <div
        id="tabpanel-cityops"
        role="tabpanel"
        aria-labelledby="tab-cityops"
        hidden={activeTab !== 'cityops'}
      >
        <CityOpsTab incidents={incidents} />
      </div>

      {/* Legend */}
      {activeTab === 'cityops' && (
        <div className="px-4 py-2.5 border-t border-black/5 dark:border-white/5 flex flex-wrap gap-3 text-[11px] text-zinc-400" aria-label="Map legend">
