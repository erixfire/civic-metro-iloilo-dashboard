    ...
    <div className="rounded-xl border border-black/10 dark:border-white/10 bg-white dark:bg-zinc-900 p-5 shadow-sm h-full">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {error   && <span className="text-[11px] text-red-400">⚠ fallback</span>}
          {!loading && !error && isFallback && <span className="text-[11px] text-amber-500">⚠ mock data</span>}
      ...
      {(criticalCount > 0 || alarmingCount > 0) && (
        <div className={`rounded-lg px-3 py-2 mb-3 text-xs font-medium flex items-center gap-2 ${
          ...
          {criticalCount > 0 && <span>{criticalCount} station{criticalCount > 1 ? 's' : ''} at Critical level</span>}
          {alarmingCount > 0 && criticalCount === 0 && <span>{alarmingCount} station{alarmingCount > 1 ? 's' : ''} at Alarming level</span>}
        </div>
      )}

      {/* Legend */}
      <div className="flex gap-3 mb-3 text-[10px] text-zinc-400">
        <span>🟢 Normal &lt;10mm/hr</span>
        <span>🟡 Alarming 10–30mm/hr</span>
        <span>🔴 Critical &gt;30mm/hr</span>
      </div>

      <div className="space-y-2">
        {gauges.length === 0 && !loading && (
          <div className="text-sm text-zinc-400 text-center py-4">No gauge data available.</div>
        )}
        {gauges.map((g) => (
          <div
            key={g.id}
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-base shrink-0">{LEVEL_ICON[g.level] ?? '🟢'}</span>
              <div className="min-w-0">
                <div className="text-xs font-medium truncate">{g.name}</div>
                <div className="text-[10px] opacity-70">{g.source}</div>
              </div>
            </div>
